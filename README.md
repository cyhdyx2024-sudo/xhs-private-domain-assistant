...
# Social CRM Copilot for Web Workbenches (第三方开源工作台效率助手)

面向网页端客服工作台的 **第三方开源 AI 对话起草辅助与知识库系统**。
...
传统客服自动回复往往机械死板，且纯全自动方案容易在关键事实或服务状态上出现偏差。本项目专为真实业务场景设计：**深度结合多轮上下文、本地知识库与人工审核，提供半自动化的回复草稿建议与线索格式化整理**。

- 🛡️ **人工主导与安全审核**：默认采用「半自动副驾」模式（仅在输入框预填草稿，需人工审核确认后发送）；对无法核验的外部动作状态（如“加了没/发了吗”）自动提示人工处理。
- 🧠 **多轮上下文理解**：结合前序对话逻辑理解客户诉求，避免机械复读与无意义推销。
- 📚 **本地业务知识库 (RAG)**：支持导入企业业务资料（PDF、Word、Markdown、FAQ 等），基于真实业务事实生成专业应答建议。
- 🎯 **线索格式化辅助**：辅助识别客户主动发送的联系方式并整理至工作台客资面板，减少手动誊抄工作量。
- 🔄 **人工案例沉淀**：操作员在工作台修改优化的话术可一键沉淀至知识库，逐步贴合品牌实际沟通风格。
- 🔑 **BYOK (Bring Your Own Key)**：支持 DeepSeek、OpenAI、Claude、GLM、Qwen、MiniMax、Moonshot 等兼容 OpenAI 接口的模型；API Key 仅保存在浏览器本地，安全可控。
- 🌙 **安全与冷却机制**：内置会话冷却、消息时限过滤与操作员活动检测，杜绝高频打扰。

---

## ⚖️ 免责声明与法律合规 (Legal Disclaimer)

1. **用途限制**：本项目仅供技术研究、交流学习及个人工作台辅助使用，**严禁用于任何形式的批量群控、恶意营销、违规引流或侵犯他人权益之行为**。
2. **合规义务**：使用者须严格遵守《网络安全法》《反不正当竞争法》《个人信息保护法》及目标平台服务条款。
3. **责任自负**：本项目为开源软件（按 AS IS 现状提供），开发者不对使用者的任何具体操作及可能产生的后果（包括但不限于账号受限、纠纷等）承担法律责任。
